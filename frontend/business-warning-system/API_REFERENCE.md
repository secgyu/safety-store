...
export function useDeleteActionPlanItem(): UseMutationResult<
  { success: boolean },
  Error,
...
export function useUpdateNotificationSettings(): UseMutationResult<
  { success: boolean; settings: NotificationSettings },
  Error,
...
function BlogList() {
  ...
  return (
    <div>
      {posts?.map((post) => (
        <Link key={post.id} to={`/blog/${post.id}`} onMouseEnter={() => handleMouseEnter(post.id)}>
          {post.title}
        </Link>
      ))}
    </div>
